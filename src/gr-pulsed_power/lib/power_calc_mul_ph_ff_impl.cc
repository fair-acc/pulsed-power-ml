 ...
 * //todo: change to 9 input vals, 11 output vals
 *
 * @param alpha A value > 0 < 1
 */
power_calc_mul_ph_ff_impl::power_calc_mul_ph_ff_impl(double alpha)
    : gr::sync_block(
          ...
          gr::io_signature::make(
              9 /* min inputs */,
              9 /* max inputs */,
              sizeof(float)), // input: voltage, current, deltaphi for all three phases
          gr::io_signature::make(
              15 /* min outputs */, 15 /*max outputs */, sizeof(float)))
{
    set_alpha(alpha);
}
 ...
 * splits them into two floats | convert back uint64_t int64 = ((long long) out[0] << 32)
 * | out[1];
 *
 ...
 */
void power_calc_mul_ph_ff_impl::calc_rms_u(float* output,
                                           const float* input,
                                           int noutput_items)
{
 ...
 * @param noutput_items The samples currently available for cumputation
 */
void power_calc_mul_ph_ff_impl::calc_rms_i(float* output,
                                           const float* input,
                                           int noutput_items)
{
    for (int i = 0; i < noutput_items; i++) {
 ...
 * @param noutput_items The samples currently available for cumputation
 */
void power_calc_mul_ph_ff_impl::calc_phi_phase_correction(float* phi_out,
                                                          const float* delta_phi,
                                                          int noutput_items)
{
    for (int i = 0; i < noutput_items; i++) {
        float temp = 0;
        if (!isnan(delta_phi[i]) && delta_phi[i] >= 0 &&
            delta_phi[i] <= 2 * M_PI) // check for valid value range -> always > 0?
        {
            temp = delta_phi[i];
 ...
 * @param noutput_items The samples currently available for cumputation
 */
void power_calc_mul_ph_ff_impl::calc_active_power(
 ...
 * @param noutput_items The samples currently available for cumputation
 */
void power_calc_mul_ph_ff_impl::calc_reactive_power(
    float* out, float* voltage, float* current, float* phi_out, int noutput_items)
{
    for (int i = 0; i < noutput_items; i++) {
 ...
 * @param noutput_items The samples currently available for cumputation
 */
void power_calc_mul_ph_ff_impl::calc_apparent_power(float* out,
                                                    float* voltage,
                                                    float* current,
                                                    int noutput_items)
{
...
void power_calc_mul_ph_ff_impl::calc_acc_val_active_power(
    float* p_out_acc, float* p_out_1, float* p_out_2, float* p_out_3, int noutput_items)
{
    for (int i = 0; i < noutput_items; i++) {
...
void power_calc_mul_ph_ff_impl::calc_acc_val_reactive_power(
    float* q_out_acc, float* q_out_1, float* q_out_2, float* q_out_3, int noutput_items)
{
    for (int i = 0; i < noutput_items; i++) {
...
void power_calc_mul_ph_ff_impl::calc_acc_val_apparent_power(float* out,
                                                            ...
                                                            int noutput_items)
{
    ...
    for (int i = 0; i < noutput_items; i++) {
        out[i] = sqrt(acc_active_power[i] * acc_active_power[i] +
                      acc_reactive_power[i] * acc_reactive_power[i]);
    }
}
