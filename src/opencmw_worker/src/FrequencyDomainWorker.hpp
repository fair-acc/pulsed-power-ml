...
private:
    ...
    struct SignalData {
        gr::pulsed_power::opencmw_freq_sink *sink;
...
public:
    ...
    explicit FrequencyDomainWorker(const BrokerType &broker)
        ...
        for (auto sink : gr::pulsed_power::globalFrequencySinksRegistry) {
            ...
            _signalsMap[completeSignalName] = SignalData({ sink, ringbuffer, poller });
...
private:
    ...
    void pollMultipleSignals(const std::string &requestedSignal, uint64_t chunksToPoll, AcquisitionSpectra &out) {
