...
// OpenGL3 (Emscripten is a C++-to-javascript compiler, used to publish
// executables for the web. See https://emscripten.org/) If you are new to Dear
// ImGui, read documentation from the docs/ folder + read the top of imgui.cpp.
// Read online: https://github.com/ocornut/imgui/tree/master/docs

// This is mostly the same code as the SDL2 + OpenGL3 example, simply with the
// modifications needed to run on Emscripten. It is possible to combine both
// code into a single source file that will compile properly on Desktop and
...
void deserialiseJson(std::string jsonString) {
  ...
  // TODO: dataAsJson is given in format R"("CounterData": {"value": 27 })"
  // which cannot be read by json::parse(). Maybe opencmw::serealiserJson is the
  // better deserialiser. For now, remove ""CounterData": " from dataAsJson to
  // make it usable with json::parse
  ...
  for (auto &element : json_obj.items()) {
    std::cout << "Json elements: key: " << element.key()
              << ", value: " << element.value() << "\n";
...
static void main_loop(void *arg) {
  ...
  if (buffer.Data.size() > 0) {
    ...
    if (ImPlot::BeginPlot("Counter Worker")) {
      ...
      ImPlot::SetupAxisLimits(ImAxis_X1, buffer.Data[bufferEnd].x - 250.0,
                              buffer.Data[bufferEnd].x, ImGuiCond_Always);
      ImPlot::SetupAxisLimits(ImAxis_Y1, 0, 100);
      ImPlot::SetNextFillStyle(IMPLOT_AUTO_COL, 0.5f);
      ImPlot::PlotLine("Counter", &buffer.Data[0].x, &buffer.Data[0].y,
                       buffer.Data.size(), 0, buffer.Offset, 2 * sizeof(float));
      ImPlot::EndPlot();
    }
    ImGui::End();
  }
