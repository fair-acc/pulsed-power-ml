...
static inline float ImPow(float x, float y) {
  return powf(x, y); // DragBehaviorT/SliderBehaviorT uses ImPow with either
                     // float/double and need the precision
}
static inline double ImPow(double x, double y) { return pow(x, y); }
static inline float ImLog(float x) {
  return logf(x); // DragBehaviorT/SliderBehaviorT uses ImLog with either
                  // float/double and need the precision
}
...
static inline float ImSign(float x) {
  return (x < 0.0f)   ? -1.0f
         : (x > 0.0f) ? 1.0f
                      : 0.0f; // Sign operator - returns -1, 0 or 1 based on
                              // sign of argument
}
...
static inline float ImFloorSigned(float f) {
  return (float)((f >= 0 || (float)(int)f == f)
                     ? (int)f
                     : (int)f - 1); // Decent replacement for floorf()
}
...
struct IMGUI_API ImRect {
  ...
  void ClipWith(const ImRect &r) {
    Min = ImMax(
        Min, r.Min); // Simple version, may lead to an inverted rectangle, which
                     // is fine for Contains/Overlaps test but not for display.
    Max = ImMin(Max, r.Max);
  }
  void ClipWithFull(const ImRect &r) {
    Min = ImClamp(Min, r.Min,
                  r.Max); // Full version, ensure both points are fully clipped.
...
template <typename T> struct ImPool {
  ...
  int GetAliveCount() const {
    return AliveCount; // Number of active/alive items in the pool (for display
                       // purpose)
  }
  int GetBufSize() const { return Buf.Size; }
  int GetMapSize() const {
    return Map.Data.Size; // It is the map we need iterate to find valid items,
                          // since we don't have "alive" storage anywhere
  }
  ...
  int GetSize() {
    return GetMapSize(); // For ImPlot: should use GetMapSize() from
                         // (IMGUI_VERSION_NUM >= 18304)
  }
#endif
};
...
struct IMGUI_API ImGuiInputTextState {
  ...
  void CursorAnimReset() {
    CursorAnim = -0.30f; // After a user-input the cursor stays on for a while
                         // without blinking
  }
  void CursorClamp() {
...
inline ImDrawList *GetForegroundDrawList(ImGuiWindow *window) {
  IM_UNUSED(
      window); // This seemingly unnecessary wrapper simplifies compatibility
               // between the 'master' and 'docking' branches.
...
inline ImGuiID GetItemID() {
  ImGuiContext &g = *GImGui; // Get ID of last item (~~ often same
...
inline void ItemSize(const ImRect &bb, float text_baseline_y = -1.0f) {
  ItemSize(bb.GetSize(),
           text_baseline_y); // FIXME: This is a misleading API since we expect
                             // CursorPos to be bb.Min.
}
...
inline ImGuiID GetFocusScope() {
  ImGuiContext &g =
      *GImGui; // Focus scope we are outputting into, set by PushFocusScope()
...
inline void FocusableItemUnregister(ImGuiWindow *window) {
  IM_ASSERT(
      0); // -> unnecessary: TempInputText() uses ImGuiInputTextFlags_MergedItem
  IM_UNUSED(window);
}
#endif
