...
int         main(int argc, char **argv) {
    ...
    std::vector<Subscription<Acquisition>>        subscriptionsTimeDomain    = { currentSubscription, signalSubscription, powerStatsSubscription, mainsFreqSubscription };
    ...
    appState.window    = SDL_CreateWindow("Pulsed Power Monitoring - Current", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, window_flags);
...
static void main_loop(void *arg) {
    ...
    ImGui::NewFrame();
    {
        ...
        ImGui::Begin("Pulsed Power Monitoring - Current", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove);
        app_header::draw_header_bar("CurrentMonitoring", args->fonts.title);
        ...
        if (ImPlot::BeginSubplots("My Subplots", rows, cols, ImVec2(-1, (window_height * 1 / 3) - 30), flags, rratios, cratios)) {
            ...
            if (ImPlot::BeginPlot("##", ImVec2(), ImPlotFlags_NoLegend)) {
                ...
                if (subscriptionsTimeDomain.size() >= 4) {
                    Plotter::plotSignals(subscriptionsTimeDomain[1].acquisition.buffers);
                }
                ImPlot::EndPlot();
                ImPlot::PopColormap();
            }
        ...
        if (ImPlot::BeginPlot("##")) {
            ...
            if (subscriptionsTimeDomain.size() > 1) {
                ...
                Plotter::plotCurrentStatistics(subscriptionsTimeDomain[0].acquisition.buffers, Interval);
            }
            ImPlot::EndPlot();
        }
