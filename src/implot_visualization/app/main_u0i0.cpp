...
int         main(int argc, char **argv) {
    ...
    // subscription for current data
    // sample rate must equal sink sample rate, F12 for constructor, (8080 -> worker, PulsedPowerservice) 
    Subscription<Acquisition>        currentSubscription("http://localhost:8080/pulsed_power/Acquisition?channelNameFilter=", { "I_0", "I_1", "I_2", "I_0_bpf", "I_1_bpf", "I_2_bpf" }, 500, 0.06 * 1000, 25.0f);
    ...
    // Subscription<Acquisition>                     mainsFreqSubscription("http://10.0.0.2:8080/pulsed_power/Acquisition?channelNameFilter=", { "mains_freq" }, sampRate, timeRange * sampRate, updateFreq);
    // Subscription<AcquisitionSpectra>              frequencySubscription("http://10.0.0.2:8080/pulsed_power_freq/AcquisitionSpectra?channelNameFilter=", { "sinus_fft" }, 50, 25 * 50, 1.0f);
    ...
    std::vector<Subscription<Acquisition>>        subscriptionsTimeDomain    = { voltageSubscription, currentSubscription, signalSubscription, powerStatsSubscription, mainsFreqSubscription };
    ...
    appState.window    = SDL_CreateWindow("Pulsed Power Monitoring - U0 I0", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 1280, 720, window_flags);
...
static void main_loop(void *arg) {
    ImGuiIO &io = ImGui::GetIO();
    // get right data from subscriptions
    ...
    ImGui::NewFrame();
    {
        ...
        ImGui::Begin("Pulsed Power Monitoring - U0 I0", nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoMove);
        app_header::draw_header_bar("voltageCurrentPhaseOne", args->fonts.title);
        ...
        if (ImPlot::BeginSubplots("My Subplots", rows, cols, ImVec2(-1, (window_height * 1 / 3) - 30), flags, rratios, cratios)) {
            ...
            if (ImPlot::BeginPlot("##", ImVec2(), ImPlotFlags_NoLegend)) {
                //subscriptionsTimeDomain    = { currentSubscription, signalSubscription, powerStatsSubscription, mainsFreqSubscription };
                if (subscriptionsTimeDomain.size() >= 5) {
                    Plotter::plotSignals(subscriptionsTimeDomain[2].acquisition.buffers);
                }
                ImPlot::EndPlot();
                ImPlot::PopColormap();
            }
            ...
            if (ImPlot::BeginPlot("##", ImVec2(), ImPlotFlags_NoLegend)) {
                if (subscriptionsTimeDomain.size() >= 4) {
                    Plotter::plotMainsFrequency(subscriptionsTimeDomain[4].acquisition.buffers, Interval);
                }
                ImPlot::EndPlot();
            }
        ...
        if (ImPlot::BeginPlot("##")) {
            ...
            if (subscriptionsTimeDomain.size() > 1) {
                printf("%s \n", subscriptionsTimeDomain[0].acquisition.buffers.data()->signalName.c_str());
                Plotter::plotU0I0(subscriptionsTimeDomain[0].acquisition.buffers, subscriptionsTimeDomain[1].acquisition.buffers, Interval);
            }
            ImPlot::EndPlot();
        }
