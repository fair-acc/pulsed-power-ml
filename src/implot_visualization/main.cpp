...
// Deserialise
// dataAsJson:  String
void deserialiseJson(std::string dataAsJson){
    ScrollingBuffer buffer;
    static int counter = 0;
    auto json_obj = json::parse(R"({"happy": true, "pi": 3.141})");
    printf("Prepared json data: %s\n", json_obj[0].get<std::string>().c_str());
    buffer.AddPoint(++counter,json_obj.value("value",0));
...
static void main_loop(void* arg)
{
    ...
    // 2. Show a simple window that we create ourselves. We use a Begin/End pair to created a named window.
    {
        ...
        ImGui::Begin("Multiple Plots in One Window");                                // Create a window
    ...
    {
        ImGui::Begin("One Plot in One Window");                                // Create a window called "Hello, world!" and append into it.
