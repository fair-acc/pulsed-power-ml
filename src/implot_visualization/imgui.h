...
struct ImVec2 {
  ...
  float operator[](size_t idx) const {
    IM_ASSERT(
        idx <=
        1); // We very rarely use this [] operator, the assert overhead is fine.
    return (&x)[idx];
  }
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_ChildMenu =
      1 << 28 // Don't use! For internal use by BeginMenu()
              // ImGuiWindowFlags_ResizeFromAnySide    = 1 << 17,  // [Obsolete]
              // --> Set io.ConfigWindowsResizeFromEdges=true and make sure
              // mouse cursors are supported by backend (io.BackendFlags &
...
template <typename T> struct ImVector {
  ...
  inline void clear_delete() {
    for (int n = 0; n < Size; n++)
      IM_DELETE(
          Data[n]); // Important: never called automatically! always explicit.
    clear();
  }
  ...
  inline void shrink(int new_size) {
    IM_ASSERT(new_size <= Size); // Resize a vector to a smaller size,
                                 // guaranteed not to cause a reallocation
    Size = new_size;
  }
...
struct ImGuiTextBuffer {
  ...
  const char *end() const {
    return Buf.Data ? &Buf.back()
                    : EmptyString; // Buf is zero-terminated, so end() will
                                   // point on the zero-terminator
  }
...
struct ImDrawListSplitter {
  ...
  inline void Clear() {
    _Current =
        0; // Do not clear Channels[] so our allocations are reused next frame
    _Count = 1;
  }
...
struct ImDrawList {
  ...
  inline void AddBezierCurve(const ImVec2 &p1, const ImVec2 &p2,
    ...
    AddBezierCubic(p1, p2, p3, p4, col, thickness,
  ...
  inline void PathBezierCurveTo(const ImVec2 &p2, const ImVec2 &p3,
    ...
    PathBezierCubicCurveTo(p2, p3, p4,
                           num_segments); // OBSOLETED in 1.80 (Jan 2021)
  }
#endif
...
struct ImFontAtlas {
  ...
  bool IsBuilt() const {
    return Fonts.Size > 0 &&
           TexReady; // Bit ambiguous: used to detect when user didn't built
                     // texture but effectively we should check TexID != 0
...
static inline void CaptureKeyboardFromApp(bool want_capture_keyboard = true) {
  SetNextFrameWantCaptureKeyboard(
      want_capture_keyboard); // Renamed as name was misleading + removed
                              // default value.
}
static inline void CaptureMouseFromApp(bool want_capture_mouse = true) {
  SetNextFrameWantCaptureMouse(
      want_capture_mouse); // Renamed as name was misleading + removed default
                           // value.
}
...
static inline void OpenPopupContextItem(const char *str_id = NULL,
  ...
  OpenPopupOnItemClick(
      str_id,
      mb); // Bool return value removed. Use IsWindowAppearing() in BeginPopup()
